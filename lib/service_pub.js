...
class ServicePub extends Component.mixin(AsyncEmitter) {
    ...
    *initialize() {
    ...
    _setupRoutes() {
        let eventSubscribe = function (req, res) {
            var info = req.body;
        ...
        let eventUnsubscribe = function (req, res) {
            var info = req.body;
            var event = req.params.event;
            if (this._subscribers[event]) {
    ...
    *triggerEvent(event, payload) {
        ...
        yield subs.map(sub => {
            return this.emitEvent(sub, payload).catch(function (e) {
    ...
    emitEvent(sub, payload) {
        return request({
            uri: "http://" + sub.uri + "/hook" + sub.hook,
            method: "POST",
            body: {
                payload: payload
            },
